...
#if defined ( ESP8266 )
SDM01::SDM01(HardwareSerial& serial, long baud, int dere_pin, int config, bool swapuart) : sdmSer(serial) {
  this->_baud = baud;
...
#elif defined ( ESP32 )
SDM01::SDM01(HardwareSerial& serial, long baud, int dere_pin, int config, int8_t rx_pin, int8_t tx_pin) : sdmSer(serial) {
  this->_baud = baud;
  this->_dere_pin = dere_pin;
  this->_config = config;
  this->_rx_pin = rx_pin;
...
#else
SDM01::SDM01(HardwareSerial& serial, long baud, int dere_pin, int config) : sdmSer(serial) {
  this->_baud = baud;
  this->_dere_pin = dere_pin;
  this->_config = config;
}
...
SDM01::SDM01(SoftwareSerial& serial, long baud, int dere_pin, int config, int8_t rx_pin, int8_t tx_pin) : sdmSer(serial) {
  this->_baud = baud;
  this->_dere_pin = dere_pin;
  this->_config = config;
  this->_rx_pin = rx_pin;
...
#else
SDM01::SDM01(SoftwareSerial& serial, long baud, int dere_pin) : sdmSer(serial) {
  this->_baud = baud;
  this->_dere_pin = dere_pin;
}
#endif
...
void SDM01::begin(void) {
  ...
  dereSet(LOW);                                                                 //set init state to receive from SDM01 -> DE Disable, /RE Enable (for control MAX485)
}

float SDM01::readVal(uint16_t reg, uint8_t node) {
  uint16_t temp;
  ...
  dereSet(HIGH);                                                                //transmit to SDM01  -> DE Enable, /RE Disable (for control MAX485)
  ...
  dereSet(LOW);                                                                 //receive from SDM01 -> DE Disable, /RE Enable (for control MAX485)
...
uint16_t SDM01::getErrCode(bool _clear) {
...
uint32_t SDM01::getErrCount(bool _clear) {
...
uint32_t SDM01::getSuccCount(bool _clear) {
...
void SDM01::clearErrCode() {
  readingerrcode = SDM_ERR_NO_ERROR;
}

void SDM01::clearErrCount() {
  readingerrcount = 0;
}

void SDM01::clearSuccCount() {
  readingsuccesscount = 0;
}

void SDM01::setMsTurnaround(uint16_t _msturnaround) {
...
void SDM01::setMsTimeout(uint16_t _mstimeout) {
...
uint16_t SDM01::getMsTurnaround() {
  return (msturnaround);
}

uint16_t SDM01::getMsTimeout() {
  return (mstimeout);
}

uint16_t SDM01::calculateCRC(uint8_t *array, uint8_t len) {
  uint16_t _crc, _flag;
  _crc = 0xFFFF;
...
void SDM01::flush(unsigned long _flushtime) {
...
void SDM01::dereSet(bool _state) {
  if (_dere_pin != NOT_A_PIN)
    digitalWrite(_dere_pin, _state);                                            //receive from SDM01 -> DE Disable, /RE Enable (for control MAX485)
}
